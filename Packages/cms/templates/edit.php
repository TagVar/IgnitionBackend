...
if ((isset($identifier)) && (!isset($_POST["submit"]))) {
    $template_query = "SELECT name, data FROM `$cms_table` WHERE id='$identifier'";
    ...
    $html = $_POST["html"];
    if (str_replace(" ", "", $name) == "") {
        $error = "You did not provide a name for this template.";
    } else {
        if ($is_edit) {
          $check_name_query = "SELECT * FROM `$cms_table` WHERE name='$name' AND type='template' AND id <> '$identifier'";
        } else {
          $check_name_query = "SELECT * FROM `$cms_table` WHERE name='$name' AND type='template'";
        } 
        $check_name_result = $connection->query($check_name_query);
        if (mysqli_num_rows($check_name_result) == 0) {
            $validated = false;
            if ($is_edit) {
                $delete_template_query = "DELETE FROM `$cms_table` WHERE id='$identifier'";
                if ($connection->query($delete_template_query)) {
                    $validated = true;
                }
            } else {
                $validated = true;
            ...
            if ($validated) {
              $add_template_query = "INSERT INTO `$cms_table` (type, name, data) VALUES ('template', '$name', '" . str_replace("'", "&#x00027;", $html) . "')";
              if ($connection->query($add_template_query)) {
                header("location: ./");
              } else {
   ...
   <div id="image-alert">
    ...
    <div id="main-container" class="content-border">
      ...
      <form action="" method="post">
        ...
        <input type="text" name="name" placeholder="Template Name" value="<?php echo $name; ?>"/>
