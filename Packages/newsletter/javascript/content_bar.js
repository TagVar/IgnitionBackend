...
$("form[name=image_form]").submit(function(event){
  if (uploadType == "upload") {
    $("input[name=image_post_subject]").val($("input[name=subject]").val());
    $("input[name=image_post_from]").val($("input[name=from]").val());
    $("input[name=image_post_bcc]").val($("input[name=bcc]").val());
    ...
    if (previousUpload.replace(" ", "") != "") {
      $("#content").val($("#content").val() +'<img src="' + document.location.origin + previousUpload + '" />');
      $(".upload-thumbnail").css("border", "0");
      previousUpload = "";
      $("#image-alert").hide();
    } else {
...
$(document).ready(function() {
    if ($("input[name=image_post_name]").val() != "") {
      $("#content").val($("#content").val() +'<img src="' + document.location.origin + "/nodes/images/" + $("input[name=image_post_name]").val() + '" />');
